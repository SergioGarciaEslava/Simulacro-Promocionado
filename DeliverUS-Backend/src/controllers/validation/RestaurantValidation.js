...
const ownerHasPromotedRestaurant = async (ownerId, promotedValue) => {
  if (promotedValue) {
    try {
      const promotedRestaurant = await Restaurant.findAll({ where: { userId: ownerId, promoted: true } })
      if (promotedRestaurant.length !== 0) {
        return Promise.reject(new Error('You can only promote one restaurant at time'))
      }
    } catch (err) {
...
const create = [
  ...
  check('promoted').custom(async (value, { req }) => { return ownerHasPromotedRestaurant(req.user.id, value) })
